<?php
$consulta = "SELECT s.cod_siniestro, s.f_siniestro as f_siniestro, 
op.f_registro as f_registro_op, s.f_registro as f_r_siniestro, s.f_denuncia as fecha_denuncia, s.lugar_siniestro, s.distrito, s.ramo_general as ramo, op.cobertura_afectada, s.cobertura as cobertura_aplicar, s.asegurado, s.cod_poliza as num_poliza, s.inicio_v as inicio_vigencia, s.fin_v as final_vigencia, op.importe_bs, op.cod_orden as num_orden, op.receptor, s.detalle_siniestro as narracion, op.doc_descargo, op.f_indemnizacion, op.concepto, op.usuario as user_reclamo,s.inspector, s.cod_cliente as codigo, op.importe_bs,op.retencion_bs,op.pago_total_bs,op.indemnizacion,s.estado, s.id_item,
GROUP_CONCAT(DISTINCT otc.cod_trabajo_compra ORDER BY otc.cod_trabajo_compra SEPARATOR ', ') 
    ...
    GROUP_CONCAT(otc.estado ORDER BY otc.estado SEPARATOR ', ') 
        AS estado_trabajos
FROM comercial.orden_pago as op INNER JOIN comercial.siniestros as s on s.cod_siniestro=op.cod_siniestro
LEFT JOIN comercial.trabajo_compra as otc on otc.cod_siniestro=s.cod_siniestro";

if (!isset($_POST['cb_lapso'])) {
    $consulta .= " WHERE op.f_registro like '%$fecha_dia%' and otc.f_registro like '%$fecha_dia%'";
    $fecha_aux = $fecha_dia;
    ...
    $consulta .= " WHERE op.f_registro >= '$fecha_inicio' and op.f_registro <= '$fecha_final' and otc.f_registro >= '$fecha_inicio' and otc.f_registro <= '$fecha_final'";
...
$consulta .= " AND s.ramo_general = 'AUTOMOTORES' GROUP BY op.cod_orden";
